...
### [B-260811-830bf6] 评估并收敛“旧格式与其他”内容支持
- 创建: 2026-08-11
- 优先级: P2
- 类型: refactor
- 改动量: L
- 问题表现: Dashboard 内容管理目前把“角色”和“Excel”归入“旧格式与其他”，但尚未确认这些格式是否仍被 Bot 运行时、命令、配置、发布包或现有用户数据实际消费。继续展示和维护会扩大导航与兼容面；未经调查直接停止支持，则可能让旧安装中的内容不可访问。当前缺少足以决定保留、隐藏、弃用或移除的代码与数据证据。
- 开发备忘: 先追踪相关内容目录、Dashboard 接口、运行时加载器、用户命令、Persona/配置流程、发布包示例、文档与测试，确认真实消费者和现存数据。基于证据比较继续完整支持、仅从 Dashboard 隐藏但保留运行时兼容、标记弃用并提供迁移提示、完全停止支持四种方案。若决定移除，明确数据保留、迁移、版本说明和失败提示；在结论确定前，不把该决策混入资源浏览布局重构。
