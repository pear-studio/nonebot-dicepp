import pytest
from module.character.base.health import HPInfo, CHAR_INFO_KEY_HP, CHAR_INFO_KEY_HP_DICE


@pytest.mark.unit
class TestBaseHPInfo:
    """base 模块 HPInfo 单元测试 — 覆盖初始化/伤害/治疗/序列化/显示"""
    ...
    def test_is_record_normal_zero_hp_unconscious(self):
        ...
        hp.is_alive = False
        assert hp.is_record_normal()  # hp_cur==0 and not alive → normal
        assert not hp.is_record_damage()
    ...
    def test_take_damage_temp_partial(self):
        ...
        hp.take_damage(10)
        assert hp.hp_temp == 0
        assert hp.hp_cur == 23  # 30 - (10 - 3) = 23
        assert hp.is_alive

    def test_take_damage_noop_when_dead(self):
        hp = HPInfo()
        hp.initialize(hp_cur=10, hp_max=40)
        hp.is_alive = False
        hp.hp_cur = -5
        hp.take_damage(10)
        assert hp.hp_cur == -5  # base: is_alive=False 时 take_damage 无操作
    ...
    def test_heal_from_negative(self):
        ...
        hp.hp_cur = -8
        hp.heal(5)
        assert hp.hp_cur == -3  # not record_normal: min(0, -8+5) = -3
    ...
    def test_long_rest_no_max(self):
        ...
        result = hp.long_rest()
        assert result == ""  # hp_max=0, 无临时HP, 无生命骰 → 空字符串
        assert hp.hp_cur == 0  # unchanged
    ...
    def test_deserialize_preserves_state(self):
        hp = HPInfo()
        hp.initialize(hp_cur=0, hp_max=40)
        hp.is_alive = False
        data = hp.serialize()
