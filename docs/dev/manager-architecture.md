...
>
> 目标架构已经由 [运行与交付简化方案](./runtime-delivery-simplification.md) 确认，但尚未实施。本页在迁移完成前继续描述当前代码事实；实施本方案时将删除或重写其中的 Manager、非空恢复与自动升级契约。
...
查询数据库以 `content/queries/*.db` 为文件边界，未出现在 `.dicepp-query-databases.json` 的数据库默认启用。Bot、Dashboard 与 Manager 共享按列名识别的四个逻辑字段 `名称、英文、来源、内容`；`名称、内容` 必需，其他额外列不参与运行时查询。每个数据库可独立启停和切换，但一次搜索只针对一个已选数据库，不再合并多个数据库或用后一个库覆盖前一个库的结果。停用不会改写 Bot 配置，运行时在下一次查询时立即生效。
