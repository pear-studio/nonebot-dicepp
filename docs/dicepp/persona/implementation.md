...
**任务**：
1. 编写 `game/decay.py` — DecayCalculator
   - `calculate(relationship) → float | None` — 返回衰减量，无需衰减返回 None
   - 逻辑：距上次互动超过 grace_period(8h) → 按 decay_rate(0.5/h) 衰减
   - 每日上限 daily_cap(5.0) 防止一觉醒来清零
   - **衰减下限**：composite_score 不低于 `初始值 + 20`（从角色卡读取初始值，默认 30 → 下限 50）
     - 衰减只能让关系从"高点"回落到"友好"，不会回到"比刚认识还差"
     - 只有用户主动做出负面行为（评分产生负 delta）才能跌破此下限
2. 在 orchestrator.chat() 开头调用 DecayCalculator
3. 如果发生衰减，记录到 score_history

**衰减配置**（PersonaConfig）：
- grace_period_hours: 8 — 免衰减期
- decay_rate_per_hour: 0.5 — 每小时衰减
- decay_daily_cap: 5.0 — 每日最大衰减

---

### Phase 3: 主动消息 + 群聊观察（约 4 天）
