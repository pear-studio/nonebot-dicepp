...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `relation_labels` | list[string] | 见下表 | 5 个关系等级标签，依次对应 冷淡[0,20) / 疏远[20,40) / 友好[40,60) / 默契[60,80) / 亲密[80,100]。 |
...
| `refuse_messages` | list[string] \| null | `null` | 信誉过低时的拒绝回复语，见下文。 |

#### `relation_labels` 默认值

如果未配置或长度不足 5 个，缺失项会回退到系统默认值：

```yaml
relation_labels: ["冷淡", "疏远", "友好", "默契", "亲密"]
```
...
当信誉分低于阈值（默认 30）时，角色会拒绝回复。该字段控制拒绝语：
...
extensions:
  persona:
    ...
    # 好感度区间标签（5个，依次对应 冷淡[0,20) / 疏远[20,40) / 友好[40,60) / 默契[60,80) / 亲密[80,100]）
...
# refuse_messages（可选，信誉过低时的拒绝回复语）
    # refuse_messages:
    #   - "……我现在不想说话。"
    #   - "（没有回应）"
```
