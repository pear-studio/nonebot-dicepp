...
class TestAppendVisibleIfActive:
    """A4: append_visible_if_active — 只在有 active session 时 append_ref"""

    async def test_no_active_session_returns_none(self, temp_db):
        """scope 无 active session 时返回 None 且不创建 session。"""
        reg = _make_registry(temp_db)
        scope = ConversationScope.for_group("g1")
        result = await reg.append_visible_if_active(scope, 99999, "assistant")
    ...
    async def test_active_session_append_ref_success(self, temp_db):
        ...
        msid1 = await temp_db.add_message_stream(
            "u1", "g1", "user", MessageType.CHAT, "hi",
        )
        conv1 = await reg.append_visible(scope, msid1, "user")
        sid1 = conv1.id

        # append_visible_if_active → 应复用 conv1 的 session
        msid2 = await temp_db.add_message_stream(
            "u1", "g1", "assistant", MessageType.PROACTIVE, "主动消息",
        )
        conv2 = await reg.append_visible_if_active(scope, msid2, "assistant")
        assert conv2 is not None
        assert conv2.id == sid1  # 同一 session

        # 确认 ref 已追加
        async with temp_db.db.execute(
            "SELECT entry_type, message_stream_id, role FROM persona_session_message "
            "WHERE session_id=? ORDER BY sequence",
            (int(sid1),),
        ) as cur:
            rows = await cur.fetchall()
        assert len(rows) == 2
        assert rows[1]["entry_type"] == "ref"
        assert rows[1]["message_stream_id"] == msid2
    ...
    async def test_cache_hit_still_works(self, temp_db):
        ...
        # 创建 active session → 进入缓存
        msid1 = await temp_db.add_message_stream("u1", "g1", "user", MessageType.CHAT, "m1")
        conv1 = await reg.append_visible(scope, msid1, "user")
        assert reg.peek_cached(scope) is not None

        # 缓存命中路径
        msid2 = await temp_db.add_message_stream("u1", "g1", "assistant", MessageType.PROACTIVE, "m2")
        conv2 = await reg.append_visible_if_active(scope, msid2, "assistant")
        assert conv2 is conv1  # 同 Python 对象

    async def test_does_not_create_new_session(self, temp_db):
        """未创建过 session 的 scope 不会新建 session。"""
        reg = _make_registry(temp_db)
        scope = ConversationScope.for_group("no-such-group")
        result = await reg.append_visible_if_active(scope, 12345, "assistant")
        assert result is None
        rows = await _active_session_rows(temp_db, scope)
        assert len(rows) == 0
