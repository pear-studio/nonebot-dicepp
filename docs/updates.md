...
1. 在 Dashboard 导出重要存档并停止 Bot。
2. 备份 `config/`、`data/`、`content/`、`dashboard/data/` 和 `manager/backups/`。
...
创建存档、清空实例和导入只接受已停止 Bot。导入目标必须为空实例，导入期间 marker 会阻止 Bot 启动；失败后清空实例并重新导入。不会迁移旧的 Manager 状态、控制 token、NapCat 或 LLOneBot 状态。
