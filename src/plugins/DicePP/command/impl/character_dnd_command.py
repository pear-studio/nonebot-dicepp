...
from character.dnd5e import DNDCharInfo, gen_template_char
        ...
        if cmd_type == CMD_TYPE_CHAR:
            ...
            if not char_info:
                ...
                if check_name == "先攻检定" and len(check_value_list) > 0:
                    try:
                        from command.impl import InitiativeCommand
    ...
    def get_help(self, keyword: str, meta: MessageMetaData) -> str:
        if keyword == "角色卡":  # help后的接着的内容
            feedback: str = ".角色卡  #查看角色卡\n" \
                            ...
                            ".角色卡模板  # 查看角色卡模板\n" \
                            "使用方法:\n" \
                            "先使用角色卡模板指令获得模板, 自行修改模板中的内容后使用角色卡记录指令记录角色卡即可\n" \
                            "示例:\n" \
                            ".角色卡记录\n" \
                            "$姓名$ 伊丽莎白\n" \
                            "$等级$ 20\n" \
                            ...
                            "每人在一个群中只能拥有一张角色卡, 但通过记录[.角色卡]指令返回的内容即可自行保存多张角色卡\n" \
            ...
            feedback: str = "$姓名$ (可选)若给定姓名, 将会在设置角色卡时自动设置玩家昵称(.nn)\n" \
                            ...
                            "$属性$ 六个用/分隔的正整数, 依此为力量,敏捷,体质,智力,感知,魅力\n" \
            ...
            feedback: str = "角色卡目前的衍生指令包括[检定, 生命骰, 长休]功能\n" \
                            ...
                            "1. [力量/敏捷/...]检定 2. [运动/体操/...]检定 3. 先攻检定 4. [力量/敏捷/...]豁免 5. [力量/敏捷/...]攻击 " \
                            "临时调整值规则和额外加值相同, 必须以[+, -, 优势, 劣势]\n" \
                            "先攻检定会将角色加入到先攻列表中\n" \
                            "示例:\n" \
                            ".运动检定\n" \
                            ".2#力量检定\n" \
