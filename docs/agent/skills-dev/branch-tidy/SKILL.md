...
- 每轮维护简短状态账本：总范围、目标 tree、提交数、已完成/当前/待整理领域和最新 backup，避免 hash 改写后丢失上下文。
...
先完成本轮只读分析再运行 `start`，不要为探索性尝试或没有净变化的候选创建新轮次。
...
保持 commit 边界和语义 patch 不变，可在同一轮调整顺序、规范 message，或同时完成两者；纯 reword 不要求实际换序。换序时检查文件存在性、定义与调用、schema、测试及构建依赖；patch 冲突或需要改写内容时停止，并改在后续 split/merge pass 解决。所有新 message 遵循项目现行提交规范。
...
任何会改写 commit、index、worktree 或 ref 的 Git 命令前，都必须确认当前分支匹配 `branch-tidy/*-work`；不匹配立即停止。

在当前 `branch-tidy/*-work` 分支手工构造新历史：

- 所有新增或重写的 message 遵循项目现行提交规范；正文必须依据实际 patch，写出标题之外的具体问题、机制和影响，不得用整理过程或 tree 等价结论代替改动说明。
...
发生冲突或工具失败后停止当前 pass，先记录 branch、HEAD、status、backup 和正在进行的 Git 操作。有标准 rebase、cherry-pick 或 merge 状态时使用对应的 abort；否则只有确认现场改动全部来自本轮后，才可验证 backup 并恢复当前 work。不得移动原分支或 backup。恢复到 clean 且 HEAD/tree 可解释后才能重试；重复出现安全错误时结束整理并汇报。
...
- 不存在后置补完、临时路线、backlog 开关、文件增删重加或相邻互逆等过程痕迹。
- 每个 scope 和 message 都与实际 patch 相符，正文包含可核查的具体事实。
- 功能领域、依赖顺序、实现与测试文档关系清楚；规模符合约束或已说明保留理由，用户给出的提交数量上限得到遵守。
- 最终 tree、线性历史和工作区状态均通过验证。

commit 数量减少是结果，不是单独的完成标准。只剩主观美化，或继续调整会增加风险、破坏数量约束时，应停止整理。完成后使用 `auto-test-run` 在最终 HEAD 做一次相称验证；测试发现的代码问题走正常开发流程，不倒灌到本次纯历史重构。
