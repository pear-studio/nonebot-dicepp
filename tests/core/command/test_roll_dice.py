...
class TestRollDice:
    ...
    async def test_multi_roll(self, h):
        await h.send_group(".r2#d20")
        await h.send_group(".r2#d20+1")
    ...
    async def test_private_roll(self, h):
        await h.send_private(".r")
        await h.send_private(".rd20")

    async def test_error_expressions(self, h):
        await h.send_group(".r(1+1)d6", checker=lambda s: len(s) > 0)
        await h.send_group(".r2(DK3)")
        await h.send_group(".rh()")
