...
from plugins.DicePP.utils import create_parent_dir
...
class QueryStore:
    ...
    async def search(
        ...
        if not self.has_database(database):
            raise QueryStoreError(f"数据库未加载: {database}")

        results = await self._search_single_db(
            database, query_tokens, fulltext, max_total + 1, 0,
        )
        total = len(results)
        if total > max_total:
