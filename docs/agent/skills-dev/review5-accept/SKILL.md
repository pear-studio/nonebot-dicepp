...
4. **Backlog 写入校验**（在逐条比对之前）：
   ...
   - 对每个 ID 运行 `python docs/agent/tools/backlog.py show <id>` 确认在 `docs/dev/backlog.md` 中存在
   - 任一缺失视为验收失败，列出差异常项并退回
5. 逐条比对：实际 diff 是否覆盖了约定的具体改点？是否有超出约定的额外改动？
   - **严重问题的测试验收**：对 `已共识·实施` 的严重问题，检查 Confirm/Reply 中是否约定了回归测试。若约定添加测试（未明确否决即视为约定）：
     - 验证测试文件是否已添加，测试内容是否覆盖了 Review 中描述的问题场景
     - 运行相关测试确认通过
     - 若测试缺失或未通过，标记为 `验收退回`，退回原因中注明"约定的回归测试缺失/未通过"
