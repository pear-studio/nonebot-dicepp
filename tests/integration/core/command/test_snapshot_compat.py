...
冲突守卫工作方式：
  1. 读取 tests/fixtures/snapshots/cmd_<name>_compat.json 中的 cases
  2. 对每个 case，使用对应命令的 CommandTextParser 解析 input
  3. 将解析结果与 expected 对比
  4. 若不一致则 fail，阻断该迁移提交（确保行为等价）

新增命令快照的步骤：
  - 在 tests/fixtures/snapshots/ 新建 cmd_<name>_compat.json
...
# ---------------------------------------------------------------------------
SNAPSHOTS_DIR = (
    find_repository_root(Path(__file__)) / "tests" / "fixtures" / "snapshots"
)
...
def test_snapshot_compat(cmd_name: str, case: Dict):
    ...
    assert actual == expected, (
        ...
        f"  expected : {expected}\n"
        f"\n若此变更为有意行为，请：\n"
        f"  1. 更新 tests/fixtures/snapshots/cmd_{cmd_name}_compat.json 中对应 case 的 expected\n"
