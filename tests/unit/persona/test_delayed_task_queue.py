...
async def test_enqueue_event_share(mock_store):
    queue = EventShareTaskQueue(mock_store)
...
async def test_tick_share_desire_below_threshold(mock_store):
    ...
    mock_store.poll_delayed_tasks.return_value = [
        DelayedTask(
            ...
            payload={"event_description": "下雨了", "reaction": "", "share_desire": 0.2},
            scheduled_at=datetime.now(),
        )
    ]
    queue = EventShareTaskQueue(mock_store, share_threshold=0.5)
...
async def test_tick_share_event(mock_store):
    ...
    mock_store.poll_delayed_tasks.return_value = [
        DelayedTask(
            id=2,
            task_type="event_share",
            payload={"event_description": "出太阳了", "reaction": "", "share_desire": 0.8},
            scheduled_at=datetime.now(),
        )
    ]
