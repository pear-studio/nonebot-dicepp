...
import roll_dice.roll_config as roll_config
from bot_utils.string import to_english_str
from roll_dice.modifier import RollExpModifier, ROLL_MODIFIERS_DICT
from roll_dice.connector import RollExpConnector, ROLL_CONNECTORS_DICT
from roll_dice.roll_utils import RollDiceError, roll_a_dice, match_outer_parentheses
from roll_dice.result import RollResult


# from config_roll import DICE_TYPE_DEFAULT
...
class RollExpressionXDY(RollExpression):
    ...
    def __init__(self, xdy_str: str):
        ...
        self.dice_type = int(type_str) if type_str else roll_config.DICE_TYPE_DEFAULT

        if self.dice_num > roll_config.DICE_NUM_MAX:
            raise RollDiceError(f"骰子数量不能大于{roll_config.DICE_NUM_MAX}")
        if self.dice_type > roll_config.DICE_TYPE_MAX:
            raise RollDiceError(f"骰子面数不能大于{roll_config.DICE_TYPE_MAX}")
...
class RollExpressionInt(RollExpression):
    ...
    def __init__(self, val: int):
        ...
        if val < roll_config.DICE_CONSTANT_MIN or val > roll_config.DICE_CONSTANT_MAX:
...
def parse_roll_exp(input_str: str, depth: int = 0) -> RollExpression:
    ...
    if depth > roll_config.PARSE_RECURSION_DEPTH_MAX:
    ...
    # print("complex", cur_str)
    left_str = cur_str
    while left_str:
        # print(cur_str, "in loop", left_str)
