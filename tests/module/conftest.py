"""
module 集成测试共享 fixture。

提供 session 级 Bot 实例和 ``h`` fixture，供移动至 module 目录的原
core/command 集成测试使用。与 core/command/conftest.py 各自独立
（不同 conftree 分支，不会冲突）。
"""
import asyncio
import uuid
...
async def bot():
    ...
    _bot_instance = Bot(f"test_module_{uuid.uuid4().hex[:12]}", readonly=True, no_tick=True)
...
@pytest.fixture
def h(bot):
    """每个测试函数独立的 _IntegrationHelper 实例。"""
    return _IntegrationHelper(bot)
