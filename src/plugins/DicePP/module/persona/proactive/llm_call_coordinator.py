...
from ..llm.router import QuotaExceeded, NonRetryableError
...
class LLMCallCoordinator:
    ...
    def _get_lock(self, target_key: str) -> asyncio.Lock:
        ...
        注意：不主动清理 _locks。asyncio.Lock 对象本身开销极小，
        target_key 数量受限于实际用户/群数量，长期运行无实际内存压力。
        """
    ...
    async def _run_loop(
        ...
        # 超过最大迭代次数（用户刷屏）
        # 若最后一轮已通过 on_result 发送，则不再重复返回
        logger.warning(
            f"coordinator: target={target_key} 超过最大迭代次数 "
            f"({self.max_iterations})，强制退出"
        )
        await self._invoke_on_exhausted(target_key, on_exhausted, last_exception)
        return None if last_result_sent else result
