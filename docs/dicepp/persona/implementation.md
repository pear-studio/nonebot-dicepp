...
1. 定义 `character/models.py` — Character 数据模型
   ...
   - CharacterBook + LoreEntry（世界书，Phase 5 使用）
...
**与 Phase 5b Drive 系统的关系**：
- Phase 2f 是简化版，基于固定规则（时间 + 好感度）
- Phase 5b Drive 系统是进阶版，基于内在驱动力（connection drive 的 frustration）
- Phase 5b 完成后可替换 Phase 2f，或两者并存（Drive 触发更个性化）
...
4. 用户档案
5. 群聊观察（Phase 2g 实现后接入，LLM 按需查询）
6. 世界书命中（Phase 5a 实现，此处预留）
