...
def serve_session(
    ...
    *,
    ...
    server.  Env changes for dashboard integration (DPP_ADMIN_HOST/PORT via
    _configure_dashboard) are likewise one-way.
    """
    _validate_loopback_host(host)
    if not 0 <= port <= 65535:
        raise ValueError("Port must be between 0 and 65535")
    if dashboard_url:
        _configure_dashboard(dashboard_url)
...
def _configure_dashboard(value: str) -> None:
    candidate = value if "://" in value else f"http://{value}"
    parsed = urlparse(candidate)
    if parsed.hostname not in {"127.0.0.1", "::1", "localhost"}:
        raise ValueError("Shell Dashboard integration only accepts a local address")
    os.environ["DPP_ADMIN_HOST"] = parsed.hostname or "127.0.0.1"
    os.environ["DPP_ADMIN_PORT"] = str(parsed.port or 4090)
