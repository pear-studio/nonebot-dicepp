...
class HubAPIClient:
    def __init__(self, base_url: str, api_key: Optional[str] = None):
        ...
        logger.info(f"[DiceHub] 初始化 API 客户端: base_url={self.base_url}, api_key={'已设置' if api_key else '未设置'}")
    ...
    async def _request(
        ...
        logger.debug(f"[DiceHub] 发起请求: {method} {url}, data={data}")
            ...
            try:
                async with aiohttp.ClientSession(timeout=self.timeout) as session:
                    async with session.request(
                        ...
                        logger.debug(f"[DiceHub] 响应: status={response.status}, content={content}")
                ...
                last_error = e
                logger.warning(f"[DiceHub] 请求失败 (尝试 {attempt + 1}/{retry}): {e}")
                if attempt < retry - 1:
                    await asyncio.sleep(1 * (attempt + 1))
                continue

        logger.error(f"[DiceHub] 请求最终失败: {last_error}")
    ...
    async def register(
        ...
        logger.info(f"[DiceHub] 注册机器人: bot_id={bot_id}, nickname={nickname}, master_id={master_id}, version={version}")
