...
### [B-260525-c7668c] old_loop.py 清除未使用的 import asyncio
- 创建: 2026-05-25
- 优先级: P2
- 类型: refactor
- 改动量: S
- 问题表现:
    - old_loop.py 中存在 import asyncio 但代码中未使用
    - 旧代码迁移至 old_loop.py 时引入的残留
- 工作计划:
    - M3 清理 old_loop.py 时一并删除 import asyncio
    - 当前 M1 阶段不动旧代码，避免引入不必要风险
