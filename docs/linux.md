...
mkdir -p config/bots data content dashboard/data manager/backups
...
Compose 会把配置、业务数据、内容、Dashboard 数据和旧存档目录挂载为可写目录：
...
创建存档、清空业务数据和导入空实例前，必须在 Dashboard 停止 Bot。导入目标必须是业务数据为空的实例；导入完成前会保留一个 marker，期间拒绝启动 Bot。管理数据库、Dashboard session、日志和程序文件不会被清空。
