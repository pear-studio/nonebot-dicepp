...
# 挂载静态资源（S5：vendor 目录用于本地 Tailwind/Alpine 副本）
_STATIC_DIR = Path(__file__).resolve().parent / "static"
_STATIC_DIR.mkdir(exist_ok=True)
(_STATIC_DIR / "vendor").mkdir(exist_ok=True)
...
async def _startup() -> None:
    ...
    # S2: admin 重启后，恢复仍在运行的 DicePP 子进程的状态缓存
    instance_manager.scan_existing_processes()
    # B2: 启动所有标记 auto_start 的实例
    instance_manager.autostart_marked_instances()


@app.on_event("shutdown")
async def _shutdown() -> None:
    # 关 admin 时不强制 kill 子进程，让 DicePP 实例继续跑（admin 重启后 scan 会恢复）
    # 用户想真正停某实例请走 admin UI 的「停止」按钮
    pass


# 注意：不再 atexit.register(stop_all) — admin 应该是 DicePP 实例的「管理器」
# 而不是「父进程」，重启 admin 不应连带 kill 所有骰娘
