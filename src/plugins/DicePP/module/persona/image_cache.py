...
class ImageCacheProtocol(Protocol):
    ...
    async def download_and_cache(self, image_meta: List[dict]) -> None: ...
...
class ImageCache:
    ...
    async def download_and_cache(self, image_meta: List[dict]) -> None:
        ...
        dice_log(
            ...
            f" cached={cached} downloaded={downloaded} failed={failed}"
        )
